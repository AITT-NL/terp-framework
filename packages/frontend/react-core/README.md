...
| `Page` | The base routed screen: a band carrying the breadcrumb trail — whose leaf **is** the view's single `h1` — plus `badges`, a `description` lead line and the `actions` cluster, then the body with loading/error slots. The band places by grid area (ADR 0135): a page with badges or a lead line spends a second row on them and the cluster joins them there at no cost in height, which leaves the trail the whole first row; a page with neither keeps the single row. Crumbs ellipsise rather than wrap, and the lead line is shown only above the second cutover. Every page renders a trail, even one of a single crumb, so an overview's title sits where a detail's does. Inside a shell the band bleeds to the content column's edge and takes `--shell-header-height`; a `measure="narrow"` frame keeps the row and drops the chrome. |
...
| `PageActions` | The page header's action cluster: `primary`, `secondary` nodes, `secondaryActions` descriptors and an `overflow` menu. The cluster follows the viewport — supporting actions drop to icons alone in the middle region and fold into the overflow menu below the first cutover — while the primary action keeps its label at every width. Only `secondaryActions` can follow the viewport: a `secondary` node's label is inside the caller's tree and can only be shown or hidden. |
...
`ConfirmDialog`; detail bodies hold `DetailList` / `DetailListGroup` / `Stack` / `Tabs` /
`ModuleNav` / `DataView` / `Card` / `Grid` / `Divider` / `Text` plus the same states. The asymmetry is
...
| `routeFieldErrors` | Split a failure's `fields` into the ones this form renders (`shown`, ready for `Field`'s `error`) and whether anything was `leftover`. The branch that matters is `leftover`: a reason naming a field the form has no input for would otherwise set state nobody reads and suppress the toast on the way out, so the user presses Save and *nothing at all* happens. |
| `randomUuid` | A cryptographically random v4 UUID in every context. `crypto.randomUUID` is secure-context-only while `lib.dom` declares it unconditionally, so calling it on a plain-http origin is a synchronous `TypeError` that type-checks cleanly and never fires on localhost. Falls back to `crypto.getRandomValues` (not secure-context-gated) for the same entropy, and throws rather than reaching for `Math.random`. Reach for it wherever the client mints an id -- an `Idempotency-Key`, an optimistic row key. |
...
| `FieldRow` | Several `Field`s side by side, with their labels, their controls and their messages each on a **shared line**. A field is as tall as its label, its control *and* whatever it has to say, so neither flex alignment is right: `align="end"` lines up the messages, so a field that grows a hint lifts its own control above its neighbours' and a bare action button sinks to the depth of the longest error on the row; `align="start"` lines up the labels, which fixes the fields and leaves the button riding above the controls. A flex row can align one **edge** of a box and the thing that has to line up here is a band in the middle of it, so the row owns three rows and each field becomes a `subgrid` of them — the instrument `DetailListGroup` already uses, for the same reason. Anything that is not a field lands on the control line, which is a placement rather than a wrapper, so a remove button keeps its own element and its own name. `gap` moves the **column** gap only: the row gap is each field's internal label-to-control measure, shared by every field on the row. Below the viewport cutover it is one column at full width — three controls and an action do not fit a phone, and that is also where the alignment problem stops existing. |
| `QuietActions` | A value with an action attached, where the action is **quiet until someone reaches for it** — a copyable identifier, a row's remove, a list item's open. Five copyable digests on one card meant five copy buttons competing with the five values they applied to. The space is **reserved, not collapsed**: the action fades rather than appears, so nothing moves when a pointer arrives. Three things make it safe rather than merely quiet — the resting state sits behind `@media (hover: hover)`, so a touch screen with no hover to reveal anything gets a visible control instead of an invisible tappable one; it reveals on `:focus-within` as well as hover, because opacity does not remove an element from the tab order and a keyboard must never land on something it cannot see; and opacity is not `visibility`, so the action is announced, named and operable throughout. The actions are a slot, the same one `Card` publishes, rather than a convention about which children count. |
...
| `DetailList` | Token-styled label/value pairs as a semantic `<dl>` (record metadata, expanded-row summaries). A real grid: `layout="aligned"` puts every label in a shared column so the values line up, `"stacked"` puts the label above its value, and `columns` takes two pairs per row. Tracks are floored at zero and long values wrap, so a 64-character digest no longer pushes the list past its container. Both non-inline layouts mute their label to the small, regular step — at the value's own size and weight a card of labelled values is a wall of bold text — and space their rows so a pair groups; `gap` moves that distance on the spacing scale, and it is the **row** gap, since the column gap is the label-to-value distance. One pair can leave the shared column with `full`, which spans it across every track with its label above its value — the escape that stops a single wide value (a paragraph, a payload) from forcing the list to split in two, since two lists measure two label columns. `columns` also takes `"auto"`, which follows the **container** rather than the viewport: a pair is at least 9rem of label and 13rem of value, each floor capped at a share of the track so one pair always fits a narrow panel instead of overflowing it. The closed one-or-two keep the reflow at the framework's viewport cutover, where `Grid` deliberately does not for a fixed count. |
| `DetailListGroup` | Several `DetailList`s that share **one** measured label column. A card that shows a record in sections renders a list per section — the sections have their own headings and their own order, and one `<dl>` cannot carry that — and each list then measures its own label column, so the values in each land on a different vertical line: four lists, four gutters, on one card. The group owns the track list and each aligned list inside becomes a `subgrid` of it, so every label in every list is measured against the same track. Explicit rather than something `Card` infers from its children: a card that wants two different label widths has to be able to say so, and a card has no business knowing about `<dl>` tracks. Children other than lists span the whole group (a `Heading` between two sections, a `Divider`), which is the other half of why it is a wrapper. It shares the measure for `layout="aligned"` at the default single column — a `columns={2}` list keeps its own four tracks rather than being quietly folded into two — above the viewport cutover, and it degrades to exactly today's output where `subgrid` is unsupported. |
