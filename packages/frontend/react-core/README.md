...
| Shell geometry (tokens, not props) | `--shell-sidebar-width-expanded` / `-collapsed`, `--shell-header-height` and `--shell-content-max-width` are published contract tokens: an app moves them from its own unlayered `theme.css` with no prop at all, and the Studio gets them from the manifest. |
| `density` (shell) | App-wide `"comfortable"` (default) or `"compact"`, stamped on the shell root; every control height and cell padding follows by token inheritance. A subtree may override it — a `DataView density="comfortable"` inside a compact shell really is comfortable, because comfortable now has named tokens and a rule rather than being the absence of an attribute. |
| Skip to content | The shell renders the skip link and owns the landmarks it skips: `main` carries `id="terp-main"` and `tabIndex={-1}` so activating the link *moves focus* rather than only scrolling, and the sidebar `aside` is named on both the desktop and drawer branches. `MAIN_CONTENT_ID` is exported for an app rendering its own chrome. |
