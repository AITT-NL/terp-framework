"""Route-level declarations a module makes about one handler.

Terp derives a request's write authority from the **HTTP method**: a safe method
(``GET`` / ``HEAD`` / ``OPTIONS``) is authorized at the read tier and marked
read-only at runtime, an unsafe one is authorized at the write tier and may
persist. That mapping is right for almost every route, and it is what
``safe_methods_are_read_only`` and the session guard enforce between them.

It has one blind spot, and :func:`read_only` is for exactly that: a handler that
...
__all__ = ["READ_ONLY_ATTRIBUTE", "is_read_only", "read_only"]
