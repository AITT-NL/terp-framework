...
export default defineConfig({
  ...
  expect: {
    toHaveScreenshot: {
      ...
      // different at all (a normalised YIQ colour distance); `maxDiffPixels` decides how many
      // counted pixels are tolerable. Only the pixel allowance used to be set here, so
      // Playwright's default `threshold: 0.2` applied — permissive enough that a status token
      // moving from `#16a34a` to `#15803d` produced *zero* differing pixels and every baseline
      // passed a change that visibly repaints every badge in every app. Phase 2d found that by
      // changing ten specimens and being told nothing had moved.
      //
      ...
      // antialiasing drift the allowance is there to absorb.
      threshold: 0.02,
      maxDiffPixels: 0,
    },
  },
