...
def test_a_group_grant_authorizes_members(
    ...
    grant = admin.post(
        ...
        json={"subject_id": group["id"], "permission": "notes.delete"},
    )
    assert grant.status_code == 201

    access = AccessService()
    assert access.has_permission(db_session, member, "notes.delete") is True
    admin.delete(f"{_GROUPS}/{group['id']}/members/{member}")
    assert access.has_permission(db_session, member, "notes.delete") is False
...
def test_deleting_a_group_cascades_over_http(
    ...
    assert AccessService().has_permission(
        db_session, member, "notes.delete"
    ) is False
