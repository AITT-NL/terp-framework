...
## Platform triage — the app-facing seams (in flight, NOT released)

A batch of findings from building on the platform, triaged against source and
worked through in tier order. **Nothing here has shipped**: it sits on
`feat/open-the-verify-profile` (framework), `feat/record-two-detector-residuals`
(spec) and `fix/dev-stack-app-env` (studio), and the release blockers are listed
at the end of this section.

The recurring shape behind half of it, worth naming because it will recur again:
*the platform delegates a generic concern to an off-the-shelf tool in its design
records, runs the tool on itself, and never wires it for apps.* Each time, the app
hand-rolls the missing check somewhere the gate does not look.

### Tier 1 — done, green locally, unmerged

- [x] **A. The verify profile is open at the app end** — `[[tool.terp.verify.checks]]`
  (ADR 0106). An app declares its own checks; they ride the profile ratchet, run
  after the platform floor, and compose into **no** assurance lane. Every branch of
  the reader fails closed, because a seam that drops what it cannot parse hands
  back a gate that is green for a check nobody ran.
- [x] **B. Dependency hygiene, blocking** — a `dependency-hygiene` check delegating
  to deptry in `full`/`release`, conditional on `[tool.deptry]`, wired into the
  template. Advisory was the status quo and is not a control.
- [x] **C. Three guide topics that taught around the platform** — `package-boundaries`
  now documents `terp check --package <sidecar>` (which has always worked; verified
  against a sidecar with injected violations), a new `secrets` topic for sealed
  config, booleans in the `forms` recipe, and a new `dependency-hygiene` topic.
- [x] **D. `env-seams` reads `.app.env.example`** — the one file in that seam a human
  maintains was the only one nothing validated. One entry per declared name, no
  others, an empty value for a declared secret (the file is committed), parsed the
  way compose's dotenv reader parses it.
- [x] **E. A throttled conformance run says so** — the 429 envelope and its
  `Retry-After` were on the wire and unread, so an exhausted rate-limit window
  surfaced as a timeout on an unrelated locator.
- [x] **F. An undrained queue is observable** — `terp outbox backlog` and an opt-in
  `GET /health/detail`. The reaper cannot answer this by construction: it scans
  *lapsed* claims, and work nobody claimed has no claim to lapse. The signal is the
  age of the oldest DUE row.
- [x] **G. Two detector residuals recorded** (terp-spec 0.28.1) — `no_manual_lease_columns`
  matches ten column spellings rather than the custody shape, and
  `no_manual_ownership_checks` gets three. Both were filed because a careful reader
  concluded from each that the rule refused a design it does not look at.
- [x] **L. Studio's Development settings reach a container** — a live defect: the
  values went into the process environment of `docker compose watch`, which compose
  reads for `${...}` interpolation only, while `env-seams` correctly refuses a
  declared name in a compose `environment:` block. Between the two halves there was
  no path at all. Studio marks the file it writes and never touches one it did not;
  `POST /development-config/import` is the escape.
- [x] **M. `terp env`** — `init`/`set`/`unset`/`list`/`check`/`example`. The manifest
  is the allow-list, a secret's value is never printed, and `example` is the
  generator half of D's parity check.
- [x] **O. The ownership refusal names routes that exist** — both halves pointed at a
  "reviewed maintenance-authority capability" that does not ship. Plus the jobs
  clause in the `ownership` guide, and the marker's spelling (it was printed with
  underscores, which the suppressor does not match).
- [x] **The confidentiality rule is in the repo** — it lived only in one maintainer's
  agent memory, so Copilot, Codex and a fresh session had never been told. See the
  "These repositories are public" section of AGENTS.md, and note that
  terp-framework had no CLAUDE.md at all, so Claude Code read none of AGENTS.md.

### Tier 2 — not started, each needs an ADR first

- [ ] **H. An out-of-gate consumer seam** — the dispatch counterpart to
  `holder_router`: claim/complete/fail over a durable queue, for a consumer that
  must not import `app` or `terp`. The custody half is precedent, not invention.
- [ ] **I. Per-route rate limits** — one fixed IP-keyed window covers every request;
  a login endpoint and an asset read should not share a bucket. Needs a declaration
  shape on `SecurityConfig`.
- [ ] **J. A `Field` envelope for the self-labelling controls** — `Switch`, `Checkbox`
  and `RadioGroup` each render their own `<label>`, so none can nest inside
  `Field`'s. `RadioGroup` is the case that carries the argument: a required choice
  can be left unset and there is nowhere for the error to go.
- [ ] **K. The sidecar package as a supported shape** — a build path, a declared rule
  *subset* so a check report stops claiming rules it could not evaluate, and the
  profile entry that runs it (from A).
- [ ] **N. Per-service delivery for a declared variable** — one `.app.env` reaches
  every backend service, so a worker's credential sits in api, migrate and seed too.
- [ ] **P. Key the ownership gate on the model, not the directory** — the clause is a
  proxy over spellings and is wrong in both directions; the false negative
  (`jobs=NAME` instead of a literal) is the worse half. Needs a record because
  ADR 0029 §8 explicitly blesses the no-policy path this clause reverses.

### Declined, with the reason recorded

- [x] **A pattern/family declaration in `environment.schema.json`** — it would make
  adding a tenant a redeploy, put N tenants' secrets in one deploy-time file, and
  dissolve "undeclared variables stay impossible by construction". Sealed config is
  the seam (ADR 0055); the gap was that no guide topic mentioned it, which C closes.

### Release blockers — why this is not shippable yet

- [ ] **CI has never run on any of it.** `ci.yml` triggers on push to `main` and on
  pull_request; the three branches are pushed but no PR is open, so every green
  above is a LOCAL green.
- [ ] **Three branches unmerged**, across framework, spec and studio.
- [ ] **The spec/framework version coupling is open.** terp-spec is at 0.28.1
  locally; the framework still pins `terp-spec==0.27.0` and `SPEC_VERSION = "0.27.0"`,
  so 0.28.0 *and* 0.28.1 are unadopted. Order matters (the two CIs are circularly
  coupled): release the spec, then repin the framework, then re-run spec CI.
- [ ] **No release commit.** The framework is at 0.11.0 with 18 branch commits and
  the 7 pre-existing ADR 0102 commits carrying no CHANGELOG entries — this repo
  writes them at release time. A release is a `chore(release): 0.12.0` that bumps
  every manifest in lockstep, writes those entries, and tags `v0.12.0`.
- [ ] **terp-studio `main` is red** — 12 failing tests in `test_api.py`, pre-existing
  and unrelated (verified against a clean tree): the expectations lag the
  `turn_blocked` / `turn_blocker` fields the API now returns.
- [ ] **`terp-core` 0.5.9 on PyPI** still carries a consumer name in its bundled
  CHANGELOG. Deletion is web-UI only and breaks the whole 0.5.9 lockstep set, since
  18 sibling distributions pin `terp-core==0.5.9`.
